...
<body>
	...
	<pre>
		* 변수 삭제(&lt;c:remove var="삭제할 변수명" scope='스코프' &gt;)
		- scope영역에서 변수를 삭제하는 테그
		- scope 미지정시 "모든" 영역에서 삭제된다.
	...
	<pre>
		* 변수 출력 (&lt; c:out value="출력할값" default="기본값" escapeXml="true/false" &gt;)
		- 데이터를 화면성에 출력할 때 사용되는 태그
		- default : value에 출력할 값이 존재하지 않는 경우 대체값을 기술한다.
		- escapeXml : 출력할 값 중 xml태그가 포함된 경우, escape처리 할지를 지정하는 속성(기본값 true)
	...
	<h3>3. 조건문 - choose(&lt; c:choose &gt; &lt; c:otherwise &gt;)</h3>
	<pre>
	...
	<h3>4. 반복문 - forEach</h3>
	<pre>
		for loop문 - c:forEach var="변수명" begin="초기값" end="끝값" step="증가값" <br>
